...
namespace Usage
{
    public class Usage
    {
        ...
        public static void ShowUsage(MainForm mainForm)
        {
            #region Designer
            ...
            mainForm.Text = "ИС ООО \"Перевозки и КО\" | Справочники | Принадлежность автомобилей";
            #endregion
        ...
        private static void populateTable(DataGridView dataGridView)
        {
            if (dataTable.Rows.Count > 0)
                dataTable.Clear();
            dataTable = Database.GetValues("Usage_List");
        ...
        private static void AddNewEntry(object sender, EventArgs e)
        {
            ...
            if (temp != null)
            {
                var command = $"INSERT INTO Usage_List (Info) VALUES (@Value2)";
        ...
        private static void DeleteEntry(object sender, EventArgs e)
        {
            if (dataGridView.SelectedRows.Count == 1)
            {
                ...
                {
                    var command = $"DELETE FROM Usage_List WHERE ID = @Value1 AND Info = @Value2";
                    var parameters = new Dictionary<string, object>
                    {
