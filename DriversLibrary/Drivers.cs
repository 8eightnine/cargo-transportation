...
namespace Drivers
{
    public class Drivers
    {
        ...
        public static void ShowDrivers(MainForm mainForm)
        {
            #region Designer
            ...
            mainForm.Text = "ИС ООО \"Перевозки и КО\" | Водители";
            #endregion
        ...
        private static void AddNewEntry(object sender, EventArgs e)
        {
            ...
            driver._isNew = 1;
            DriverForm fo = new DriverForm(driver);
            fo.ShowDialog();
            populateTable(dataGridView);
        }
        private static void EditEntry(object sender, EventArgs e)
        {
            if (dataGridView.SelectedRows.Count == 1)
            {
                ...
                string command = $"SELECT * FROM 'Driver' WHERE ID = {dr.ItemArray[0]}";
                Database.ReadData("Databases\\make.db", command, dt);
                DataRow dr2 = dt.Rows[0];
                Driver driver;
                driver = Driver.ParseToOrder(dr2);
                DriverForm fo = new DriverForm(driver);
                fo.ShowDialog();
            }
            populateTable(dataGridView);
        ...
        private static void DeleteEntry(object sender, EventArgs e)
        {
            if (dataGridView.SelectedRows.Count == 1)
            {
                ...
                {
                    Database.DeleteDriver(Int32.Parse(dataGridView.Rows[rowIndex].Cells[0].Value.ToString()), dataGridView.Rows[rowIndex].Cells[3].Value.ToString());
                    populateTable(dataGridView);
                }
            }
            else
