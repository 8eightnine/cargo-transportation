...
namespace cargo_transportation.Classes
{
    public static class ErrorHandler
    {
        ...
        public static void DllLoadingError(string data, string dll, string function)
        {
            ...
            else if (data.Contains("класс"))
            {
                MessageBox.Show($"Ошибка при загрузке библиотеки. Библиотека повреждена или подлежит обновлению.\nНазвание библиотеки: {dll}.dll");
            }
            else
            {
                MessageBox.Show($"Неопознанная ошибка при загрузке библиотеки. Сообщите полный текст ошибки разработчику. Текст ошибки: {data}");
            }
        }
