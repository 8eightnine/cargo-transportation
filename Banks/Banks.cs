...
namespace Banks
{
    public class Banks
    {
        ...
        public static void ShowBanks(MainForm mainForm)
        {
            #region Designer
            ...
            mainForm.Text = "ИС ООО \"Перевозки и КО\" | Справочники | Банки";
            #endregion
        ...
        private static void populateTable(DataGridView dataGridView)
        {
            if (dataTable.Rows.Count > 0)
                dataTable.Clear();
            dataTable = Database.GetBankInfo("Bank");
        ...
        private static void DeleteEntry(object sender, EventArgs e)
        {
            if (dataGridView.SelectedRows.Count == 1)
            {
                ...
                DialogResult dialogResult = MessageBox.Show($"Вы хотите удалить следующую строку:\n {dataGridView.Rows[rowIndex].Cells[0].Value.ToString()} - {dataGridView.Rows[rowIndex].Cells[1].Value.ToString()}", "Подтвердите удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                if (dialogResult == DialogResult.Yes)
                {
                    var command = $"DELETE FROM Bank WHERE ID = @Value1 AND Name = @Value2";
                    var parameters = new Dictionary<string, object>
                    {
                        { "@Value1", Int32.Parse(dataGridView.Rows[rowIndex].Cells[0].Value.ToString())},
                        { "@Value2", dataGridView.Rows[rowIndex].Cells[1].Value.ToString() }
                    };
